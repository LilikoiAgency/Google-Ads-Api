...
export default function PacingReportsView({
  ...
  useEffect(() => {
    fetch(`${apiBase}/config`).then((r) => r.json()).then((j) => {
      if (j?.data) { setConfig(j.data); setConfigDraft(j.data); }
    }).finally(() => setLoading((l) => ({ ...l, config: false })));

    fetch(`${apiBase}/reports`).then((r) => r.json()).then((j) => {
  ...
  useEffect(() => {
    ...
    setPreviewHtml(null);
    fetch(`${apiBase}/reports/${activeId}`)
      .then((r) => r.json())
  ...
  async function handlePreview() {
    ...
    try {
      const res = await fetch(`${apiBase}/preview`, { method: "POST" });
  ...
  async function handleSendNow() {
    ...
    try {
      const res = await fetch(`${apiBase}/send-now`, { method: "POST" });
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || `HTTP ${res.status}`);
      ...
      const listRes = await fetch(`${apiBase}/reports`);
    ...
    <div className="pacing-page" style={{ color: C.textPri, fontFamily: "Inter, system-ui, sans-serif", minHeight: "100vh" }}>
      ...
      <div className="pacing-page-header">
        <div>
          ...
          <p style={{ margin: "4px 0 0", color: C.textSec, fontSize: 13 }}>{subtitle}</p>
        </div>
      ...
      <div className="pacing-main-grid">
        ...
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          {view === 'trends' ? (
            <PacingTrendChart endpoint={`${apiBase}/trends`} />
          ) : loading.active ? (
